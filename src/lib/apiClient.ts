...
export const getIdeas = (): Promise<Idea[]> =>
  fetch(`${API_BASE_URL}/ideas`).then(res => handleResponse<Idea[]>(res));
export const getTeams = (): Promise<Team[]> =>
  fetch(`${API_BASE_URL}/teams`).then(res => handleResponse<Team[]>(res));
export const getUsers = (): Promise<User[]> =>
  fetch(`${API_BASE_URL}/users`).then(res => handleResponse<User[]>(res));
export const getLeaderboardData = (): Promise<{ users: User[], ideas: Idea[] }> =>
  fetch(`${API_BASE_URL}/leaderboard`).then(res => handleResponse<{ users: User[], ideas: Idea[] }>(res));
export const getIdeaById = (id: string): Promise<{ idea: Idea; author: User; team: Team | undefined; teamMembers: User[]; joinRequesters: User[] }> =>
  fetch(`${API_BASE_URL}/ideas/${id}`).then(res => handleResponse<{ idea: Idea; author: User; team: Team | undefined; teamMembers: User[]; joinRequesters: User[] }>(res));
...
export const getNotifications = (userId: string): Promise<Notification[]> =>
  fetch(`${API_BASE_URL}/notifications/${userId}`).then(res => handleResponse<Notification[]>(res));
